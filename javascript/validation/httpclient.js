...
window.onbeforeunload = function() {
    if (preserveCaptivePortal != "N" && keepaliverequest === "Y" && getState() === States.SIGNED_IN && !isClientHandheld()) {
        var b = "mode=193&username=" + encodeURIComponent(document.getElementById("username").value) + "&a=" + (new Date()).getTime();
...
function getRedirectUrl() {
    if (redirectRequestedUrl.localeCompare("true") == 0) {
        if (redirect_url) {
            var b = /^http:\/\/|^https:\/\/|^ftp:\/\//i;
            var a = redirect_url.match(b);
            if (a.length != 0) {
                return redirect_url
            } else {
                return ""
